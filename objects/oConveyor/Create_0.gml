...
moveObjects=function(_xDir,_yDir) {
	...
	for (var i=0;i<ds_list_size(objs);i++)
	{
		...
		if _i.object_index==ply show_debug_message(delta_time)
		with _i
		{
			x+=_xMove;
			y+=_yMove;
			if object_index==oGrapple&&place_meeting(x,y,oConveyor)
			{
				...
				xDir=0;
				yDir=0;
				if _xDir==-1 //this is terrible but I couldn't get one uniform case to work
				{
				...
				//with ply while place_meeting(x,y-2000,oConveyor)
				//{
				//	y-=sign(oGrapple.xDir*_c.image_speed);
				//	x+=sign(oGrapple.yDir*_c.image_speed);
				//}
				ply.hsp=0;
				ply.vsp=0;
			}
