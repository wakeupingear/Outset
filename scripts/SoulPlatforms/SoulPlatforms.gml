...
/*function activateAdjacentSoulPlatforms(status){
	var _list=ds_list_create();
	instance_place_list(x-1,y,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++) _list[|i].active=status;
	instance_place_list(x+1,y,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++) _list[|i].active=status;
	instance_place_list(x,y-1,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++) _list[|i].active=status;
	instance_place_list(x,y+1,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++) _list[|i].active=status;
...
function checkAdjacentSoulRetract(){
	var _list=ds_list_create();
	instance_place_list(x-1,y,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++) if _list[|i].image_xscale==0||_list[|i].image_yscale==0
	{
		ds_list_destroy(_list);
		return false;
	}
...
function setSoulParent(){
	var _list=ds_list_create();
	instance_place_list(x-1,y,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++)
	{
		_list[|i].parent=id;
		with _list[|i] setSoulParent();
	}
	instance_place_list(x+1,y,oSoulBranch,_list,false);
	for (var i=0;i<ds_list_size(_list);i++)
	{
